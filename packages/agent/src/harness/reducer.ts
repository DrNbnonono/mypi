...
function hasRunId(
  record: LaneRecord,
): record is Exclude<LaneRecord, OperationStartedRecord> & { runId: string } {
  return "runId" in record && typeof record.runId === "string";
}

function matchesProvisionedEntry(
  entry: Entry,
...
function validateExactProvisionedEntry(
...
function validateAttemptReason(record: StepAttemptRecord): void {
  ...
  if (record.step === "compaction") {
    ...
    corrupt(
      "invalid_compaction_reason",
      `${record.step} attempt ${record.id} has a compaction reason`,
    );
  }
}
...
function validateAttemptSequence(
  ...
  const previousResult = previousRecord
    ? entriesById.get(previousRecord.resultEntryId)
    : undefined;
  ...
  if (record.resultEntryId !== previousRecord.resultEntryId) {
    corrupt(
      "inconsistent_step",
      `${record.step} attempts disagree on their result entry id`,
    );
  }
  if (record.compactionReason !== previousRecord.compactionReason) {
    corrupt(
      "inconsistent_step",
      `${record.step} attempts disagree on their compaction reason`,
    );
  }
}
...
function validateToolStart(
  ...
  if (
    !toolCall ||
    toolCall.id !== record.toolCallId ||
    toolCall.name !== record.toolName
  ) {
    corrupt(
      "tool_call_mismatch",
...
export function validateRecordLog(input: RecordLogSlice): void {
  ...
  const queueEnqueues = new Map<
    string,
    Extract<LaneRecord, { type: "queue_enqueued" }>
  >();
  ...
  for (const record of records) {
    ...
    if (hasRunId(record)) {
      if (!starts.has(record.runId)) {
        corrupt(
          "unknown_operation",
          `Record ${record.id} references unknown operation ${record.runId}`,
        );
      }
      ...
      if (finishSeq !== undefined && record.seq > finishSeq) {
        corrupt(
          "record_after_finish",
          `Record ${record.id} follows the finish of operation ${record.runId}`,
        );
      }
    }
    ...
    switch (record.type) {
      ...
      case "step_attempt":
        ...
        validateAttemptSequence(
          record,
          latestAttempt.get(record.runId),
          entriesById,
        );
      ...
      case "queue_enqueued":
        if (
          ...
          corrupt(
            "queue_after_abort",
            `${record.queue} item ${record.target.id} was enqueued after abort`,
          );
        }
...
function deriveToolBatch(
  ...
  const calls = toolCalls.map((toolCall, toolIndex) => {
    ...
    return {
      ...
      ...(result?.type === "message" && result.terminate === true
        ? { terminate: true }
        : {}),
    };
  });
...
export function reduceLaneState(
  input: LaneReductionInput,
): LaneReductionResult {
  ...
  const cancelledQueueIds = new Set(
    records
      .filter((record) => record.type === "queue_cancelled")
  ...
  const capturedInitialMessageIds = new Set(
    started?.intent.kind === "run"
      ? started.intent.initialMessages.map((target) => target.id)
      : [],
  );
  const pendingNextRun = pendingQueueRecords
    .filter(
      (record) =>
        record.queue === "nextRun" &&
        !capturedInitialMessageIds.has(record.target.id),
    )
  ...
  const operationRecords = records.filter((record) =>
    record.type === "operation_started"
      ...
      : "runId" in record && record.runId === started.id,
  );
  const aborting = operationRecords.some(
    (record) => record.type === "abort_requested",
  );
  const pendingSteer = aborting
    ? []
    : pendingQueueRecords
        .filter(
          (record) => record.queue === "steer" && record.runId === started.id,
        )
        .map((record) => clone(record.target));
  const pendingFollowUp = aborting
    ...
    : pendingQueueRecords
        .filter(
          (record) =>
            record.queue === "followUp" && record.runId === started.id,
        )
        .map((record) => clone(record.target));
  ...
  const missingInitialMessages =
    started.intent.kind === "run"
      ? started.intent.initialMessages
          .filter((target) => !entriesById.has(target.id))
          .map(clone)
      : [];

  const newestAttempt = operationRecords
    .filter((record) => record.type === "step_attempt")
    .at(-1);
  const step =
    newestAttempt && !entriesById.has(newestAttempt.resultEntryId)
      ? {
          ...
          ...(newestAttempt.step === "compaction"
            ? { compactionReason: newestAttempt.compactionReason }
            : {}),
        }
      : null;
  ...
  for (const id of consumedInputIds) {
    ...
    if (entry?.type === "message")
      newestConsumedInputSequence = Math.max(
  ...
  const deferredWriteIds = new Set(
    operationRecords
      .filter((record) => record.type === "write_deferred")
  ...
  if (
    ...
    const producedByStep = operationRecords.some(
      (record) =>
        record.type === "step_attempt" &&
        record.resultEntryId === newestOwnEntry.id,
    );
    ...
    const producedByDeferredFetch =
      operationRecords.some(
        (record) =>
          record.type === "usage" &&
          record.cause === "deferred_fetch" &&
          record.entryId === newestOwnEntry.id,
      ) ||
