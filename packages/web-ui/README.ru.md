...
Локальный браузерный интерфейс для [Pi Agent Harness](https://github.com/earendil-works/pi). Pi Web использует те же локальные настройки и файлы сессий, что и pi, поэтому в браузере можно запускать Coding или Sec агента, настраивать модели и ресурсы, просматривать файлы проекта и контролируемое состояние безопасности.
...
- **Интерфейс на английском и упрощённом китайском**: при первом запуске Pi Web учитывает язык браузера, а затем язык можно переключить в верхней панели. Элементы управления и структурированные статусы Sec workspace также переключаются; вывод модели и инструментов сохраняет язык, возвращённый средой выполнения.
