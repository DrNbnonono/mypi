...
class FakeBus implements SubagentEventBus {
	...
	emit(event: string, payload: unknown): void {
		if (event === SUBAGENT_DELEGATION_REQUEST_EVENT) {
			...
			queueMicrotask(() => this.emit(SUBAGENT_DELEGATION_RESPONSE_EVENT, {
				requestId: request.requestId,
				ownerRunId: request.ownerRunId,
				nodeId: request.nodeId,
				status: "completed",
				result: { kind: "structured", value: { status: "completed", summary: "reviewed", observations: ["fact"], evidence: [{ summary: "evidence", confidence: 0.9 }], gaps: [], proposedActions: ["next"] } },
				usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, cost: 0, turns: 1, toolCalls: 1, durationMs: 2 },
			}));
		}
...
describe("pi-subagents structured delegation bridge", () => {
	it("keeps child execution bounded and returns structured evidence proposals", async () => {
		...
		state.ctfProfile = {
			kind: "reverse", objective: state.goal, recommendedCapabilities: ["binary-triage", "reverse-analysis"],
			flagPatterns: [], expectedEvidence: ["control-flow clue"], createdAt: "2026-08-30T00:00:00Z",
		};
	...
	it("upserts delegation lifecycle records instead of duplicating them", () => {
		...
		const planned: SecurityDelegationRecord = { id: "delegation-1", role: "sec-analysis", objective: "fixture", status: "planned", evidenceIds: [], createdAt: "2026-08-30T00:00:00Z" };
		state = applySecurityEvent(state, { type: "delegation_recorded", delegation: planned, createdAt: planned.createdAt });
		state = applySecurityEvent(state, { type: "delegation_recorded", delegation: { ...planned, status: "completed", completedAt: "2026-08-30T00:01:00Z" }, createdAt: "2026-08-30T00:01:00Z" });
