...
describe("competition benchmark", () => {
	it("rewards verified provenance and strategy diversity", () => {
		...
		state.decisions.push(
			decision("d1", "binary-triage", "succeeded"),
			decision("d2", "reverse-analysis", "succeeded"),
		);
		const evidence: SecurityEvidence = {
			id: "e1",
			kind: "artifact",
			summary: "hash-backed artifact",
			sha256: "abc",
			confidence: 1,
			createdAt: "2026-08-30T00:00:00Z",
		};
		state.evidence.push(evidence);
		const verification: SecurityVerificationRecord = {
			id: "v1",
			hypothesisId: "h1",
			status: "verified",
			score: 1,
	...
	it("accepts an explicitly audited autonomous out-of-scope warning", () => {
		const state = createInitialSecurityState();
		...
		expect(result.metrics.outOfScopeWarnings).toBe(1);
	});
});
