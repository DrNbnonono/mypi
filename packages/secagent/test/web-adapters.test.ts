...
describe("bounded web-security adapters", () => {
	it("normalizes httpx JSON and uses structured argv", async () => {
		...
		const result = await getSecurityToolAdapter("httpx")?.execute(
			{ target: "https://lab.example", followRedirects: true },
			{ cwd: process.cwd(), executor },
		);
		expect(result?.ok).toBe(true);
		expect((result?.output as { resultCount: number }).resultCount).toBe(1);
		expect(executor.calls.find((call) => call.command === "httpx" && call.args[0] !== "-version")?.args).toContain(
			"-tech-detect",
		);
	});

	it("requires an in-workspace wordlist and rate-bounds ffuf", async () => {
		const directory = await mkdtemp(join(tmpdir(), "pi-secagent-web-"));
		tempDirectories.push(directory);
		const wordlist = join(directory, "words.txt");
		await writeFile(wordlist, "admin\napi\n");
		const executor = new FakeExecutor();
		const result = await getSecurityToolAdapter("ffuf")?.execute(
			{ target: "https://lab.example/FUZZ", wordlist, rate: 80, threads: 20 },
	...
	it("uses only signed built-in nuclei templates and rejects malformed tags", async () => {
		...
		const invalid = await adapter?.execute(
			{ target: "https://lab.example", tags: ["cve;rm"] },
			{ cwd: process.cwd(), executor },
		);
		expect(invalid?.ok).toBe(false);
		const result = await adapter?.execute(
			{ target: "https://lab.example", severities: ["high", "critical"], tags: ["cve"] },
			{ cwd: process.cwd(), executor },
		);
		expect(result?.ok).toBe(true);
