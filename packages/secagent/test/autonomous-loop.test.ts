...
class ScenarioExecutor implements SecurityToolExecutor {
	...
	run(command: string, args: readonly string[]): Promise<SecurityToolCommandResult> {
		...
		if (versionCheck)
			return Promise.resolve({ stdout: `${command} 1.0`, stderr: "", exitCode: 0, timedOut: false, durationMs: 1 });
		if (this.failOnce.delete(command))
		...
		const stdout =
			command === "curl"
				...
				: command === "httpx"
					? '{"url":"http://127.0.0.1","status_code":200,"title":"lab"}'
					: command === "nuclei"
						? '{"template-id":"fixture","severity":"medium","matched-at":"http://127.0.0.1"}'
						: command === "nmap"
							? "Nmap scan report\n80/tcp open http"
							: command === "file"
								? "sample.bin: ELF 64-bit LSB executable"
								: command === "strings"
									? "ELF\nflag-like-marker\n"
									: command === "readelf"
										...
										: command === "objdump"
											...
											: command === "binwalk"
												...
												: command === "exiftool"
													? '[{"FileName":"sample.bin","FileType":"ELF"}]'
...
function appendInitial(runtime: SecAgentRuntime, state: SecurityState): void {
	...
	if (state.scope.targets.length)
		runtime.append({ type: "scope_set", scope: state.scope, createdAt: state.task.createdAt });
	if (state.ctfProfile)
		runtime.append({ type: "ctf_profiled", profile: state.ctfProfile, createdAt: state.task.createdAt });
	runtime.append({ type: "stage_changed", stage: state.stage, createdAt: state.task.createdAt });
}

async function artifactState(
	scenario: Extract<SecurityScenario, "reverse-engineering" | "incident-response" | "ctf">,
	ctfKind?: "pwn" | "reverse",
): Promise<{ state: SecurityState; cwd: string }> {
	...
	state.task = {
		id: `task-${scenario}`,
		goal: `controlled ${scenario}`,
		scenario,
		assets: [{ id: "artifact", name: "sample.bin", kind: "unknown", path: "sample.bin" }],
		constraints: ["controlled fixture"],
		successCriteria: ["collect reproducible evidence"],
	...
	if (scenario === "ctf")
		state.ctfProfile = {
			kind: ctfKind ?? "reverse",
			objective: state.goal,
			recommendedCapabilities: ["artifact-triage", "binary-triage", "reverse-analysis"],
			flagPatterns: ["flag{...}"],
			expectedEvidence: ["artifact facts"],
...
describe("autonomous state-space loop", () => {
	it("progresses through distinct Web strategies and records gateway evidence", async () => {
		...
		state.task = {
			id: "web",
			goal: "map controlled web fixture",
			scenario: "web-security",
			assets: [],
			constraints: [],
			successCriteria: ["collect HTTP evidence"],
			declaredAuthorization: ["loopback"],
			pendingConfirmations: [],
			createdAt: "2026-08-30T00:00:00Z",
		};
		state.goal = state.task.goal;
		state.stage = "recon";
		state.scope = {
			targets: [{ id: "scope", kind: "url", value: "http://127.0.0.1:18080/" }],
			authorizationSource: "test",
			updatedAt: state.task.createdAt,
		};
		const runtime = new SecAgentRuntime(new MemoryStore());
		appendInitial(runtime, state);
		const executor = new ScenarioExecutor();
		const result = await new AutonomousSearchLoop(runtime, new SecurityExecutionGateway(runtime)).run(
		...
		const selected = runtime
			.snapshot()
			.state.decisions.map(
				(decision) => decision.candidates.find((candidate) => candidate.id === decision.selectedActionId)?.tool,
			);
	...
	it("moves away from a failed killchain strategy and records a replan", async () => {
		...
		state.task = {
			id: "killchain",
			goal: "controlled killchain",
			scenario: "penetration-test",
			assets: [],
			constraints: [],
			successCriteria: ["advance after failure"],
			declaredAuthorization: ["loopback"],
			pendingConfirmations: [],
			createdAt: "2026-08-30T00:00:00Z",
		};
		...
		appendInitial(runtime, state);
		const executor = new ScenarioExecutor();
		executor.failNext("httpx");
		await new AutonomousSearchLoop(runtime, new SecurityExecutionGateway(runtime)).run(
			{ cwd: process.cwd(), executor },
			{ maxSteps: 3 },
		);
		...
		const strategies = decisions.map(
			(decision) => decision.candidates.find((candidate) => candidate.id === decision.selectedActionId)?.capability,
		);
		expect(new Set(strategies.filter(Boolean)).size).toBeGreaterThan(1);
	});

	it.each(["reverse-engineering", "incident-response"] as const)(
		"runs bounded local artifact progression for %s",
		async (scenario) => {
			...
			expect(
				executor.calls.some((tool) =>
					["file", "strings", "readelf", "objdump", "binwalk", "exiftool"].includes(tool),
				),
			).toBe(true);
		},
	);
