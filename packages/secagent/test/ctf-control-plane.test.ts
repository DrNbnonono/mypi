...
function failed(id: string): SecurityDecision {
	return {
		...
		candidates: [{ id: `${id}-a`, tool: "curl", description: "repeat", goalRelevance: 1, informationGain: 1, confidence: 1, cost: 0.1, preconditions: [], risk: 0.1, score: 0.8 }],
		selectedActionId: `${id}-a`, resultStatus: "failed", actualResult: "no progress",
	};
}

describe("CTF control plane", () => {
	...
	it("profiles CTF capabilities and chooses the CTF specialist", () => {
		...
		expect(recommendation.role).toBe("ctf-specialist");
		expect(recommendation.envelope.budget.maxToolCalls).toBeGreaterThan(0);
	});

	it("observer detects repeated failed strategy", () => {
		const state = createInitialSecurityState();
