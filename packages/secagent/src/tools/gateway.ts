...
export class SecurityExecutionGateway {
	...
	constructor(runtime: SecAgentRuntime) { this.runtime = runtime; }
	...
	async execute(request: SecurityGatewayRequest, context: SecurityGatewayContext): Promise<SecurityToolExecutionResult> {
		...
		const audit: ToolAuditRecord = {
			id: randomUUID(), toolCallId: `gateway-${randomUUID()}`, toolName: request.tool, decisionId: request.decisionId,
			createdAt: startedAt, risk, scope, policyMode: state.policyMode, policyDecision: permission, blocked: false,
		...
		const finish = (result: SecurityToolExecutionResult, blocked = false): SecurityToolExecutionResult => {
			audit.completedAt = new Date().toISOString(); audit.blocked = blocked; audit.isError = !result.ok;
			...
			this.runtime.appendAudit(audit); return result;
		};
		const finishDecision = (result: SecurityToolExecutionResult, blocked = false): SecurityToolExecutionResult => {
			...
			this.runtime.append({ type: "decision_completed", decisionId: request.decisionId,
				actualResult: audit.resultSummary ?? (result.ok ? "tool completed" : "tool failed"), status: result.ok ? "succeeded" : "failed",
		...
		if (decision.resultStatus && decision.resultStatus !== "pending") return finish(failed(`Decision ${decision.id} is already ${decision.resultStatus}`), true);
		const selected = decision.candidates.find((candidate) => candidate.id === decision.selectedActionId);
		if (!selected || selected.tool.toLowerCase() !== request.tool.toLowerCase()) return finish(failed(`Decision ${decision.id} does not authorize tool ${request.tool}`), true);
		...
		if (permission === "confirm") {
			...
			const approved = await context.confirm(`SecAgent ${risk.level} approval`, `${request.tool}\n${audit.inputSummary}\n\nRisk: ${risk.reasons.join(", ")}\nScope: ${scope.reasons.join(", ")}`);
		...
		if (!availability.available) return finishDecision({ ok: false, diagnostic: availability.diagnostic, evidence: [] });
		...
		for (const normalized of result.evidence) {
			const evidence: SecurityEvidence = {
				id: randomUUID(), kind: normalized.kind ?? "observation", summary: normalized.summary, source: normalized.source,
				sha256: normalized.sha256, confidence: normalized.confidence, decisionIds: [request.decisionId],
				targetRefs: normalized.targetRefs ? [...normalized.targetRefs] : undefined, createdAt: new Date().toISOString(),
			};
