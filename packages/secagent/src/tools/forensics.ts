...
function normalizeBinwalk(output: Record<string, unknown>): Record<string, unknown> {
	...
	return {
		...output,
		scanOnly: true,
		hits: lines.filter((line) => /^\s*\d+\s+0x[0-9a-f]+\s+/i.test(line)).slice(0, 256),
	};
}
...
async function executeForensics(
	metadata: SecurityToolMetadata,
	command: ForensicsToolName,
...
export function createForensicsAdapter(
	metadata: SecurityToolMetadata,
	command: ForensicsToolName,
): SecurityToolAdapter {
	return {
		...
		checkAvailability: (context) =>
			checkCommandAvailability(command, context, command === "exiftool" ? ["-ver"] : undefined),
