...
export function createSecAgentStaticAnalysisExtension(runtime: SecAgentRuntime): SecAgentInlineExtension {
	return defineSecAgentExtension("secagent-static-analysis", (pi) => {
		...
		pi.registerTool<typeof StaticExecuteParams, SecurityToolExecutionResult>({
			...
			description:
				"Execute constrained read-only binary or forensic artifact analysis through the same decision, policy, evidence, and audit gateway.",
			promptSnippet:
				"security_static_execute: run bounded read-only artifact analysis for an existing planned decision",
