...
import { type GeneratedCandidateSet, generateCandidateActions } from "./core/candidate-generator.ts";
...
export function createSecAgentCandidateExtension(runtime: SecAgentRuntime): SecAgentInlineExtension {
	return defineSecAgentExtension("secagent-candidates", (pi) => {
		...
		pi.registerTool<typeof CandidateGenerationParams, GeneratedCandidateSet>({
			...
			description:
				"Generate deterministic runnable action candidates from SecAgent state instead of relying only on model-self-reported alternatives.",
