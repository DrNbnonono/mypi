...
export function buildSecurityReportMarkdown(
	...
	const lines = [
		...
		...(document.evidenceGraph.verifications.length
			? document.evidenceGraph.verifications.map(
					(item) =>
						`- ${item.id} ${item.status} hypothesis=${item.hypothesisId} score=${item.score.toFixed(3)} sources=${item.independentSources}: ${item.reason}`,
				)
		...
		...(document.findings.length
			? document.findings.map(
					(item) =>
						`- ${item.id} [${item.severity}] verified=${item.verified === true}: ${item.summary}${item.remediation ? `; remediation: ${item.remediation}` : ""}`,
				)
		...
		...(document.replans.length
			? document.replans.map(
					(item) =>
						`- ${item.createdAt} ${item.trigger}${item.previousDecisionId ? ` after ${item.previousDecisionId}` : ""}: ${item.reason}`,
				)
			: ["No replans recorded."]),
		"",
		"## Observer Trace",
	...
	for (const decision of document.decisions) {
		...
		for (const candidate of decision.candidates)
			lines.push(
				`  - ${candidate.id}: tool=${candidate.tool} score=${candidate.score.toFixed(4)} risk=${candidate.risk.toFixed(2)} cost=${candidate.cost.toFixed(2)} novelty=${candidate.noveltyPenalty?.toFixed(2) ?? "0"} budget=${candidate.budgetPenalty?.toFixed(2) ?? "0"}`,
			);
		lines.push("");
	}
