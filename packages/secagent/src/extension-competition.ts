...
const CandidateParams = Type.Object({
	id: Type.String({ minLength: 1 }), tool: Type.String({ minLength: 1 }), description: Type.String({ minLength: 1 }),
	goalRelevance: Type.Number({ minimum: 0, maximum: 1 }), informationGain: Type.Number({ minimum: 0, maximum: 1 }),
	confidence: Type.Number({ minimum: 0, maximum: 1 }), riskHint: Type.Optional(Type.Number({ minimum: 0, maximum: 1 })),
	cost: Type.Number({ minimum: 0, maximum: 1 }), preconditions: Type.Optional(Type.Array(Type.String())),
	capability: Type.Optional(Type.String()), targets: Type.Optional(Type.Array(Type.String())),
	expectedEvidence: Type.Optional(Type.Array(Type.String())), successCriteria: Type.Optional(Type.Array(Type.String())),
	stopConditions: Type.Optional(Type.Array(Type.String())), fallbackActionIds: Type.Optional(Type.Array(Type.String())),
	estimatedDurationMs: Type.Optional(Type.Number({ minimum: 0 })),
});
const PlanParams = Type.Object({
	candidates: Type.Array(CandidateParams, { minItems: 2, maxItems: 8 }),
	evidenceIds: Type.Optional(Type.Array(Type.String())), rationale: Type.Optional(Type.String()), expectedResult: Type.Optional(Type.String()),
});
const EvidenceParams = Type.Object({
	...
	kind: Type.Optional(StringEnum(["observation", "artifact", "indicator", "finding"] as const)), summary: Type.Optional(Type.String()),
	source: Type.Optional(Type.String()), sha256: Type.Optional(Type.String()), confidence: Type.Optional(Type.Number({ minimum: 0, maximum: 1 })),
	decisionId: Type.Optional(Type.String()), fromEvidenceId: Type.Optional(Type.String()), toEvidenceId: Type.Optional(Type.String()),
	toHypothesisId: Type.Optional(Type.String()), relation: Type.Optional(StringEnum(["supports", "contradicts", "derived-from", "duplicates", "verifies"] as const)),
	verificationId: Type.Optional(Type.String()), severity: Type.Optional(StringEnum(["info", "low", "medium", "high", "critical"] as const)),
	remediation: Type.Optional(Type.String()),
});
const BudgetParams = Type.Object({
	action: StringEnum(["show", "set"] as const), maxDecisions: Type.Optional(Type.Number({ minimum: 1, maximum: 1000 })),
	maxToolCalls: Type.Optional(Type.Number({ minimum: 1, maximum: 10000 })), maxReplans: Type.Optional(Type.Number({ minimum: 1, maximum: 1000 })),
...
const DelegateParams = Type.Object({ objective: Type.Optional(Type.String()), parentDecisionId: Type.Optional(Type.String()) });
...
function candidateInput(value: {
	id: string; tool: string; description: string; goalRelevance: number; informationGain: number; confidence: number; riskHint?: number;
	cost: number; preconditions?: string[]; capability?: string; targets?: string[]; expectedEvidence?: string[]; successCriteria?: string[];
	stopConditions?: string[]; fallbackActionIds?: string[]; estimatedDurationMs?: number;
}): CandidateActionInput {
...
function registerCompetitionTools(pi: ExtensionAPI, runtime: SecAgentRuntime): void {
	...
	pi.registerTool<typeof PlanParams, SecurityDecision | undefined>({
		name: "security_plan", label: "Security Plan",
		description: "Rank candidate actions using goal progress, information gain, confidence, risk, novelty, and budget pressure.",
		promptSnippet: "security_plan: persist an auditable plan or replan decision", parameters: PlanParams,
		async execute(_id, params) {
			...
			const decision: SecurityDecision = {
				id: randomUUID(), createdAt: new Date().toISOString(), goal: current.goal, stage: current.stage,
				evidenceIds: params.evidenceIds ?? [], candidates: ranked, selectedActionId: selected.id,
				rationale: params.rationale?.trim() || `Selected highest utility candidate; score=${selected.score.toFixed(4)}`,
				expectedResult: params.expectedResult?.trim() || selected.expectedEvidence?.join("; "), resultStatus: "pending",
				planRevision: current.decisions.length + 1, attempt: current.decisions.filter((item) => item.goal === current.goal).length + 1,
				replanId, budgetSnapshot: structuredClone(current.budget),
			};
			...
			return textResult(`Selected ${selected.id} (${selected.tool}) score=${selected.score.toFixed(4)}${replanId ? ` after replan ${replanId}` : ""}`, decision);
		},
	});

	pi.registerTool<typeof EvidenceParams, unknown>({
		name: "security_evidence", label: "Security Evidence",
		description: "Build a provenance graph, verify hypotheses, and enforce verified findings.",
		promptSnippet: "security_evidence: connect facts to hypotheses and verify them", parameters: EvidenceParams,
		async execute(_id, params) {
			const state = runtime.snapshot().state;
			if (params.action === "show_graph")
				return textResult(`evidence=${state.evidence.length}; edges=${state.evidenceGraph.edges.length}; graphErrors=${graphIntegrityErrors(state).length}`, state.evidenceGraph);
			if (params.action === "add_evidence" && params.summary?.trim()) {
				const evidence: SecurityEvidence = {
					id: randomUUID(), kind: (params.kind ?? "observation") as EvidenceKind, summary: params.summary.trim(),
					source: params.source?.trim() || undefined, sha256: params.sha256?.trim() || undefined, confidence: params.confidence ?? 0.8,
					decisionIds: params.decisionId ? [params.decisionId] : undefined, createdAt: new Date().toISOString(),
				};
			...
			if (params.action === "link" && params.fromEvidenceId && params.relation && (params.toEvidenceId || params.toHypothesisId)) {
				try {
					const edge = createEvidenceEdge({ fromEvidenceId: params.fromEvidenceId, toEvidenceId: params.toEvidenceId,
						toHypothesisId: params.toHypothesisId, relation: params.relation as EvidenceRelation, confidence: params.confidence });
					runtime.append({ type: "evidence_linked", edge, createdAt: edge.createdAt });
					return textResult(`Evidence edge ${edge.id} recorded`, edge);
				} catch (error) { return errorResult(`Error: ${error instanceof Error ? error.message : String(error)}`); }
			}
			...
			if (params.action === "create_finding" && params.verificationId && params.summary?.trim()) {
				...
				const finding: SecurityFinding = {
					id: randomUUID(), summary: params.summary.trim(), severity: params.severity ?? "medium",
					evidenceIds: verification?.evidenceIds ?? [], verificationId: params.verificationId, verified: true,
					remediation: params.remediation?.trim() || undefined, createdAt: new Date().toISOString(),
				};
	...
	pi.registerTool<typeof BudgetParams, SecurityState["budget"]>({
		name: "security_budget", label: "Security Budget", description: "Configure bounded decision, tool-call and replan budgets.",
		promptSnippet: "security_budget: bound autonomous execution", parameters: BudgetParams,
		async execute(_id, params) {
			if (params.action === "set") {
				const limits = normalizeBudgetLimits({ maxDecisions: params.maxDecisions, maxToolCalls: params.maxToolCalls,
					maxReplans: params.maxReplans, deadlineAt: params.deadlineAt });
	...
	pi.registerTool<typeof ObserverParams, unknown>({
		name: "security_observer", label: "Security Observer",
		description: "Sidecar supervision for drift/stall, compact memory and external termination checks.",
		promptSnippet: "security_observer: supervise progress without taking over execution", parameters: ObserverParams,
		async execute() {
			const state = runtime.snapshot().state;
			const signals = observeSecurityState(state);
			const existing = new Set(state.observerSignals.map((item) => `${item.kind}:${item.reason}`));
			for (const item of signals) if (!existing.has(`${item.kind}:${item.reason}`))
	...
	pi.registerTool<typeof CtfParams, unknown>({
		name: "security_ctf", label: "Security CTF", description: "Profile an authorized CTF and expose capability-oriented guidance.",
		promptSnippet: "security_ctf: classify the challenge and select capability families", parameters: CtfParams,
		async execute(_id, params) {
	...
	pi.registerTool<typeof DelegateParams, unknown>({
		name: "security_delegate", label: "Security Delegate", description: "Create a bounded specialist task envelope from scope, evidence and budget.",
		promptSnippet: "security_delegate: dispatch a bounded specialist", parameters: DelegateParams,
		async execute(_id, params) {
	...
	pi.on("tool_call", async (event) => {
		if (event.toolName === "security_decide") return { block: true, reason: "Use security_plan so novelty, budget and replan state are recorded." };
		if (event.toolName === "security_state") {
			...
			if (input.action === "add_finding" || input.action === "add_hypothesis" || input.action === "reject_hypothesis")
				return { block: true, reason: "Use security_evidence so hypotheses and findings pass through the verifier." };
		}
		return undefined;
	});
}
