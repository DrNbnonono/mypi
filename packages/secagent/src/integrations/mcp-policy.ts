...
const READ_ONLY_TOOL_HINT =
	/(?:^|_)(?:get|list|read|search|query|fetch|inspect|describe|status|view|lookup|find)(?:_|$)/i;
const MUTATING_TOOL_HINT =
	/(?:^|_)(?:create|update|write|set|patch|upload|send|merge|deploy|publish|change|modify)(?:_|$)/i;
const HIGH_RISK_TOOL_HINT =
	/(?:^|_)(?:delete|remove|destroy|drop|terminate|kill|wipe|reset|revoke|disable|execute|exec|shell|command|exploit)(?:_|$)/i;
...
function addTextTargets(value: string, targets: Set<string>, allowBareDomain: boolean): void {
	...
	for (const match of value.matchAll(IPV4_PATTERN)) {
		...
		if (isIP(candidate.split("/")[0] ?? candidate) === 4) targets.add(candidate);
	}
	if (allowBareDomain) for (const match of value.matchAll(DOMAIN_PATTERN)) targets.add(match[0]);
}
...
export function extractMcpNetworkTargets(input: Record<string, unknown>): string[] {
	...
	visit("args" in input ? parseArgs(input.args) : input, targets, undefined, 0, { remaining: 256 });
	return [...targets];
}
...
export function resolveMcpProxyCall(input: Record<string, unknown>): ToolResolution {
	...
	if (!delegatedTool) {
		const connecting = typeof input.connect === "string" && input.connect.trim() !== "";
		return {
			known: true,
			resolvedTools: ["mcp"],
			capabilities: [connecting ? "mcp.connect" : "mcp.discover"],
			baseRisk: connecting ? "P1" : "P0",
			requiresScope: false,
			reasons: [connecting ? "MCP connection operation" : "MCP metadata/discovery operation"],
		};
	}
