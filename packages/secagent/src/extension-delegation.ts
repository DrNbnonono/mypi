...
export function createSecAgentDelegationExtension(runtime: SecAgentRuntime): InlineExtension {
	return {
		name: "secagent-delegation",
		factory: (pi) => {
			...
			pi.registerTool<typeof DelegateRunParams, unknown>({
				...
				description: "Run a bounded existing SecAgent specialist through pi-subagents structured delegation. Child side-effectful/network tools are blocked; returned evidence and proposals re-enter the parent SecurityState.",
				...
				async execute(_id, params, _signal, _update, ctx) {
					...
					if (consumedToolCalls > 0)
						runtime.append({ type: "budget_consumed", resource: "tool-call", amount: consumedToolCalls, createdAt: completedAt });
					const details = { recommendation, requestId: request.requestId, response, result, evidenceIds };
					if (response.status !== "completed" || !result)
						return {
							content: [{ type: "text", text: `Delegation ${response.status}: ${response.error ?? "no structured result"}` }],
							isError: true,
							details,
						};
